...
const schema = z.object({
  ...
  // Rich-text HTML from the composer now, not plain markdown-lite text --
  // see the same field on the campaign send route for why the cap is
  // higher than a plain-text body would need.
  body: z.string().trim().min(1, "Add a message body first.").max(30000),
